...
import javaswingdev.ButtonItem;
import javaswingdev.MenuEvent;
import javaswingdev.ModelMenuItem;
import javaswingdev.PanelMenu;
...
public class MenuCustom extends javax.swing.JComponent {
    ...
    public void addItem(String... menus) {
        if (menus.length > 0) {
            PanelMenu2 menu = new PanelMenu2(this, layout, getFont(), getComponentCount(), menus);
    ...
    protected void hideShowMenu(Component... coms) {
        for (Component com : getComponents()) {
            if (!check(com, coms)) {
                ((PanelMenu2) com).hideShowingMenu();
            }
        }
    }
    ...
    public void applay(JPanel panel, JPanel panelInicial) {
    ...
    layer.add(panelInicial);
    // Agrega el JLayeredPane al JPanel
    ...
    public String getMenuNameAt(int index, int subMenu) {
        PanelMenu2 menu = (PanelMenu2) getComponent(index);
